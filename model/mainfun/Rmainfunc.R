...
data = read.csv("E:\\DownloadMusicR\\model\\mainfun\\traindata.csv",header = FALSE)
library(caTools)
...
while(length(line <- readLines(f,n=1)) > 0) {
  root <- "E:\\DownloadMusicR\\model\\mainfun\\"
  
  ClaimsTest = read.csv(paste(root, line, sep = ""),header = FALSE)
  ...
  ss <- subset(RR,mainMelody==1)
  
  staff <-ss[,1]
  
  noquote(paste( line ,staff, collapse=","))
